// Classic theme specific effects
import { EffectPresets as BaseEffectPresets, EffectsHelper } from '../../shared/effects.js';

// You can extend the base effect presets with classic-specific effects
export const EffectPresets = {
    ...BaseEffectPresets,
    classicRetro: {
        ...BaseEffectPresets.retro,
        glowIntensity: 0.9,
        colorShift: 0.3,
        blinkRate: 1.5,
        // Additional classic-specific effect parameters
        vintageFilter: true,
        scanlines: {
            enabled: true,
            intensity: 0.2,
...
export const ThemeEffectsHelper = {
    ...
    applyThemeEffect(ctx, element, intensity = 1, theme) {
        // Apply base retro effect if it exists
        if (this.applyRetroEffect) {
            this.applyRetroEffect(ctx, element, intensity);
        }

        // Add classic-specific scanline effect if enabled
        if (theme?.visualEffects?.scanlines?.enabled) {
            this.applyScanlines(ctx,
                theme.visualEffects.scanlines.intensity,
                theme.visualEffects.scanlines.count);
        }
    },

    applyScanlines(ctx, intensity = 0.2, count = 50) {
